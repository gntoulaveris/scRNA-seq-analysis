...
```{r}
data1_seurat <- RunPCA(data1_seurat, features = VariableFeatures(object = data1_seurat))
data1_seurat <- RunUMAP(data1_seurat, dims = 1:44)

umap_embeddings <- as.matrix(data1_seurat@reductions$umap@cell.embeddings)
...
data1_seurat <- ProjectDim(data1_seurat, reduction = "umap", dims = 1:2)
...
gmm_model <- Mclust(umap_embeddings[ ,1:2], G = 1:10)
cluster_ids <- gmm_model$classification
...
ggparcoord(posterior_data, columns = 3:ncol(posterior_data), groupColumn = "cluster_label",
           ...
           scale_color_viridis(discrete=FALSE) +
           labs(x = "Posterior Probabilities", y = "Probability Value", color = "Cluster Label") +
           theme(axis.title = element_text(margin = margin(t = 10, r = 0, b = 10, l = 0)))
...
```
```{r}
# heatmap of the 10 most variable genes
DoHeatmap(data1_seurat, features = VariableFeatures(data1_seurat)[1:10], cells = 1:197, 
                           size = 4, angle = 90) 
...
VlnPlot(data1_seurat, features = VariableFeatures(data1_seurat)[1:10])
...
ggplot(joint_probs, aes(x = joint_prob)) +
...
```{r}
joint_probs2 <- posterior_data_long %>%
  group_by(cell_id, cluster_label) %>%
  summarise(joint_prob = prod(posterior_prob))

# Plot the joint probability distribution for each cluster
ggplot(joint_probs2, aes(x = joint_prob, fill = cluster_label)) +
  geom_density(alpha = 0.5) +
  labs(x = "Joint Probability", y = "Density", title = "Joint Probability Distribution by Cluster") +
  theme_minimal() +
...
data1_seurat <- RunPCA(data1_seurat, features = VariableFeatures(object = data1_seurat))
data1_seurat <- RunTSNE(data1_seurat)

# extract the PCA cell embeddings
tsne_embeddings <- as.matrix(data1_seurat@reductions$tsne@cell.embeddings)
...
# perform GMM clustering and choose best model according to the BIC criterion
gmm_model <- Mclust(tsne_embeddings[ ,1:2], G = 1:10)
cluster_ids <- gmm_model$classification
